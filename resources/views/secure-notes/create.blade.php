...
@section('title', 'Nueva Nota Segura')

@section('content')
<div class="max-w-2xl mx-auto bg-white p-8 rounded-lg shadow-md">
    <h2 class="text-2xl font-bold text-gray-800 mb-6 border-b pb-2">Nueva Nota Segura</h2>

    <form action="{{ route('secure-notes.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div>
            ...
            <input type="text" name="title" value="{{ old('title') }}"
                ...
                placeholder="Ej. Recuperación de cuenta, PIN, etc.">
        ...
        <div>
            ...
            <textarea name="content_encrypted" rows="8"
                class="w-full px-4 py-2 border rounded-md focus:ring-2 focus:ring-blue-500 focus:outline-none @error('content_encrypted') border-red-500 @enderror"
                placeholder="Escribe aquí tu información protegida...">{{ old('content_encrypted') }}</textarea>
        ...
        <div>
            ...
            <select name="folder_id"
                ...
                @foreach($folders as $folder)
                    <option value="{{ $folder->id }}" {{ old('folder_id', $current_folder_id ?? '') == $folder->id ? 'selected' : '' }}>
                        {{ $folder->name }}
                    </option>
                @endforeach
            </select>
